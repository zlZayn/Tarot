...
2. 运行中服务器：对 base_url 逐个 GET 全部纹理与入口页，断言全 200（防止路径坏了只显示黑屏/白模）

用法:
    python tests/run_checks.py                # 仅文件清单对比（不需要服务器）
    python tests/run_checks.py http://localhost:8000   # 清单对比 + HTTP 全量校验
